...
#if AP_CRSF_SCRIPTING_ENABLED
AP_CRSF_Telem * check_AP_CRSF_Telem(lua_State *L) {
    AP_CRSF_Telem * ud = AP_CRSF_Telem::get_singleton();
    if (ud == nullptr) {
        // This error will never return, so there is no danger of returning a nullptr
        not_supported_error(L, 1, "crsf");
    }
    return ud;
}
#endif // AP_CRSF_SCRIPTING_ENABLED
...
#if AP_FENCE_ENABLED
static int AC_Fence_get_enabled_fences(lua_State *L) {
    binding_argcheck(L, 1);
    AC_Fence * ud = check_AC_Fence(L);
    const uint8_t data = static_cast<uint8_t>(ud->get_enabled_fences());
...
#if AP_FENCE_ENABLED
static int AC_Fence_present(lua_State *L) {
    binding_argcheck(L, 1);
    AC_Fence * ud = check_AC_Fence(L);
    const uint8_t data = static_cast<uint8_t>(ud->present());
...
#if AP_FENCE_ENABLED
static int AC_Fence_get_safe_alt_max_m(lua_State *L) {
    binding_argcheck(L, 1);
    AC_Fence * ud = check_AC_Fence(L);
    const float data = static_cast<float>(ud->get_safe_alt_max_m());
...
#if AP_FENCE_ENABLED
static int AC_Fence_get_safe_alt_min_m(lua_State *L) {
    binding_argcheck(L, 1);
    AC_Fence * ud = check_AC_Fence(L);
    const float data = static_cast<float>(ud->get_safe_alt_min_m());
...
#if AP_FENCE_ENABLED
static int AC_Fence_get_breach_direction_NED(lua_State *L) {
    binding_argcheck(L, 2);
    AC_Fence * ud = check_AC_Fence(L);
    const uint8_t raw_data_2 = get_uint8_t(L, 2);
    const uint8_t data_2 = static_cast<uint8_t>(raw_data_2);
    Vector3f data_5003 {};
    Location data_5004 {};
    const bool data = static_cast<bool>(ud->get_breach_direction_NED(
            data_2,
            data_5003,
            data_5004));
...
#if AP_FOLLOW_ENABLED && (APM_BUILD_TYPE(APM_BUILD_ArduPlane)||APM_BUILD_COPTER_OR_HELI||APM_BUILD_TYPE(APM_BUILD_Rover))
static int AP_Follow_get_target_dist_and_vel_NED_m(lua_State *L) {
    binding_argcheck(L, 1);
    AP_Follow * ud = check_AP_Follow(L);
    Vector3f data_5002 {};
    Vector3f data_5003 {};
    Vector3f data_5004 {};
    const bool data = static_cast<bool>(ud->get_target_dist_and_vel_NED_m(
            data_5002,
            data_5003,
            data_5004));
...
#if AP_FOLLOW_ENABLED && (APM_BUILD_TYPE(APM_BUILD_ArduPlane)||APM_BUILD_COPTER_OR_HELI||APM_BUILD_TYPE(APM_BUILD_Rover))
static int AP_Follow_get_target_sysid(lua_State *L) {
    binding_argcheck(L, 1);
    AP_Follow * ud = check_AP_Follow(L);
    const uint32_t data = static_cast<uint32_t>(ud->get_target_sysid());
...
#if AP_SCRIPTING_BINDING_MOTORS_ENABLED && (APM_BUILD_TYPE(APM_BUILD_ArduPlane)||APM_BUILD_COPTER_OR_HELI)
...
#endif // AP_SCRIPTING_BINDING_MOTORS_ENABLED && (APM_BUILD_TYPE(APM_BUILD_ArduPlane)||APM_BUILD_COPTER_OR_HELI)
...
static int AP_Periph_FW_get_vehicle_state(lua_State *L) {
    binding_argcheck(L, 1);
    AP_Periph_FW * ud = check_AP_Periph_FW(L);
    const uint64_t &data = ud->get_vehicle_state();
...
#if AP_CRSF_SCRIPTING_ENABLED
static int AP_CRSF_Telem_send_response(lua_State *L) {
    binding_argcheck(L, 1);
    AP_CRSF_Telem * ud = check_AP_CRSF_Telem(L);
    ud->get_semaphore().take_blocking();
    const bool data = static_cast<bool>(ud->send_response());
...
#if AP_CRSF_SCRIPTING_ENABLED
static int AP_CRSF_Telem_pop_menu_event(lua_State *L) {
    binding_argcheck(L, 1);
    AP_CRSF_Telem * ud = check_AP_CRSF_Telem(L);
    ud->get_semaphore().take_blocking();
    ud->pop_menu_event();
...
#if AP_AHRS_ENABLED
static int AP_AHRS_airspeed_EAS(lua_State *L) {
    binding_argcheck(L, 1);
    AP_AHRS * ud = check_AP_AHRS(L);
    float data_5002 {};
    ud->get_semaphore().take_blocking();
    const bool data = static_cast<bool>(ud->airspeed_EAS(
            data_5002));
...
const luaL_Reg AC_Fence_meta[] = {
    {"get_enabled_fences", AC_Fence_get_enabled_fences},
    {"present", AC_Fence_present},
    {"get_safe_alt_max", AC_Fence_get_safe_alt_max_m},
    {"get_safe_alt_min", AC_Fence_get_safe_alt_min_m},
    {"get_breach_direction_NED", AC_Fence_get_breach_direction_NED},
...
const luaL_Reg AP_Follow_meta[] = {
    {"get_target_dist_and_vel_NED_m", AP_Follow_get_target_dist_and_vel_NED_m},
    ...
    {"get_target_sysid", AP_Follow_get_target_sysid},
...
#if AP_CRSF_SCRIPTING_ENABLED
const luaL_Reg AP_CRSF_Telem_meta[] = {
    {"send_response", AP_CRSF_Telem_send_response},
    {"pop_menu_event", AP_CRSF_Telem_pop_menu_event},
    {"send_write_response", lua_CRSF_send_response},
    {"peek_menu_event", lua_CRSF_peek_menu_event},
    {"get_menu_event", lua_CRSF_get_menu_event},
    {"add_menu", lua_CRSF_add_root_menu},
};

static int AP_CRSF_Telem_index(lua_State *L) {
    return load_function(L,AP_CRSF_Telem_meta,ARRAY_SIZE(AP_CRSF_Telem_meta));
}
#endif // AP_CRSF_SCRIPTING_ENABLED
...
const luaL_Reg AP_AHRS_meta[] = {
    ...
    {"airspeed_EAS", AP_AHRS_airspeed_EAS},
    ...
    {"airspeed_estimate", AP_AHRS_airspeed_EAS},
...
static int AP_ESC_Telem_Backend__TelemetryData_power_percentage(lua_State *L) {
    AP_ESC_Telem_Backend::TelemetryData *ud = check_AP_ESC_Telem_Backend__TelemetryData(L, 1);
    binding_argcheck(L, 2);
    const uint8_t raw_data_2 = get_uint8_t(L, 2);
    const uint8_t data_2 = static_cast<uint8_t>(raw_data_2);
    ud->power_percentage = data_2;
    return 0;
}
...
static int AP_Scripting_SerialAccess_set_unbuffered_writes(lua_State *L) {
    binding_argcheck(L, 2);
    AP_Scripting_SerialAccess * ud = check_AP_Scripting_SerialAccess(L, 1);
    const bool data_2 = static_cast<bool>(lua_toboolean(L, 2));
    ud->set_unbuffered_writes(
            data_2);
...
static int Location_set_alt_m(lua_State *L) {
    binding_argcheck(L, 3);
    Location * ud = check_Location(L, 1);
    const float raw_data_2 = luaL_checknumber(L, 2);
    const float data_2 = raw_data_2;
    const lua_Integer raw_data_3 = get_integer(L, 3, static_cast<int32_t>(Location::AltFrame::ABSOLUTE), static_cast<int32_t>(Location::AltFrame::ABOVE_TERRAIN));
    const Location::AltFrame data_3 = static_cast<Location::AltFrame>(raw_data_3);
    ud->set_alt_m(
            data_2,
            data_3);
...
#if AP_EXTENDED_ESC_TELEM_ENABLED
    {"power_percentage", AP_ESC_Telem_Backend__TelemetryData_power_percentage},
#endif // AP_EXTENDED_ESC_TELEM_ENABLED
...
const luaL_Reg AP_Scripting_SerialAccess_meta[] = {
    {"set_unbuffered_writes", AP_Scripting_SerialAccess_set_unbuffered_writes},
